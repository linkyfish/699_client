...
function callBackOnMessage(res) {
    ...
    switch(data['cmd']) {
        ...
        case 'afterSend':
            ...
            var content = whisper.replaceContent(data.data.msg);
...
function showMessage(info) {
    ...
    var content = whisper.replaceContent(info.content);
        ...
        if(0 == res.code && res.data.length > 0){
            ...
            for(var i = 0; i < len; i++){
                ...
                if('mine' == item.type){
                    ...
                    _html += '<div class="layim-chat-text">' + whisper.replaceContent(item.content) + ' </div>' + ifRead + '</li>';

                }else {

                    var readFlag = '<div class="layim-chat-text check-read" data-msg-id="' + item.log_id + '">' + whisper.replaceContent(item.content) + '</div>';
                    if (2 == item.read_flag) {
                        readFlag = '<div class="layim-chat-text check-read" data-msg-id="' + item.log_id + '">' + whisper.replaceContent(item.content) + '</div>';
                    }
