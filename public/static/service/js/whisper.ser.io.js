...
$(function () {
    ...
    $("#praise").on('click', function () {
        ...
        socket.emit("praiseKf", JSON.stringify({
            ...
            if (0 == data.code) {
                ...
                $(".chat-box").append(whisper.showSystem("已发送评价"));
    ...
    $(".word").hover(function () {
        ...
        $("#content").html(whisper.replaceContent(whisper.replaceEdit(($(this).attr("data-content")))));
    }, function () {
        $("#word-box").hide();
    });
...
function sendWord(obj) {
    ...
    sendMessage(whisper.replaceEdit($(obj).attr('data-content')));
    $("#word-box").hide();
}
...
function addUser(customer, newInFlag) {
    ...
    var chatTpl =
        '<div data-name="'+customer.customer_id+'" class="chat-box" id="ct-' + customer.customer_id + '" ' + display + '></div>'
    ;;//.join('');
...
function showMessage(data) {
    ...
    // 展示聊天信息
    var chatMsg = whisper.showMessage(data);
    ...
    if ('ws' == protocol) {
        ...
        socket.emit('chatMessage', JSON.stringify({
            ...
            if (400 == data.code) {
                ...
                var msg = whisper.send(input, kefuUser.avatar, data.data);
            } else if (0 == data.code) {
                var msg = whisper.send(input, kefuUser.avatar, data.data);
            }
        ...
        $.post('/index/api/send2Customer', {
            ...
            var msg = whisper.send(input, kefuUser.avatar, res.data);
            if (0 == nowModel) {
                $("#ct-" + activeUser).append(msg);
        ...
        if(0 == res.code && res.data.length > 0){
            ...
            $.each(res.data, function (k, v) {
                if(v.type == 'mine') {
                    ...
                    _html += whisper.showMessage({time: v.create_time, avatar: v.from_avatar, content: v.content, chat_log_id: v.log_id, read_flag: v.read_flag,customer_id:v.from_id});
                }
            });
